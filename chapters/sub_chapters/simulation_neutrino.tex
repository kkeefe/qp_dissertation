...
The value of this integral gives the percentage of events which would have been fully captured with the given buffer length.
...
True neutrino interaction energy is notoriously difficult to reconstruct.
The neutrino itself carries no charge and its track is not directly reconstructed in the LAr.
For this reason we choose to demonstrate how the resets and ASIC FIFO depths vary both by the energy deposited into the LAr as well as the true neutrino energy.

Figure~\ref{fig:example_asic_energy_comparison} shows the relationship between the energy deposited and the max FIFO depth as well as the total APA resets.
There is a large variance on the points (not shown) due to the large uncertainty in how much of the true energy deposited 
...
\begin{subfigure}{.5\textwidth}
  ...
  \caption{}
\end{subfigure}
\caption{A comparison of ASIC resets vs the energy deposited, instead of the true Lepton incident energy.}
