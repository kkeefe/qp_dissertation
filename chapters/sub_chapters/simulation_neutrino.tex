...
Further details on radiogenic backgrounds can be found at~\citep{DUNE-FD_TDRv4:Abi_2020, ar39_backgrounds, phd_backgrounds}
...
The time recorded for the reset is the same as the time the simulation calculates the 6250 electron arriving at the designated pixel region.
This is an optimization I added to the existing simulation framework for these resets.
...
There will always be some delay between the final electron's arrival and the time of the analog reset.
...
This figure shows the total resets in the 10 second window for the 16$\times$16 tile array.
The other tile size configurations (4$\times$4, 8$\times$8, 10$\times$14) also use these reset data for background noise.
All of the tiles start with the top left node as their base node.
}
...
There is a large cluster of resets which occur for all pixels at $\approx$ 10 seconds.
...
\begin{tabular}{|| p{30mm} | p{30mm} | p{90mm} ||}
 ...
 \hline
  Horn Current & Forward and Reverse & Beam horn current direction affects neutrino flux, as shown in Figure~\ref{fig:neutrino_flux}. \\
 \hline
 ...
 \hline
  Momentum Angle & 0\unit{\degree}, $\pm$2\unit{\degree}, $\pm$90\unit{\degree} & Different momentum angles are different Z-positions, in general, direct ionized particle tracks within the active volume. \\
 \hline
\end{tabular}
