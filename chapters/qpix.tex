...
\subsection{Resets, RTDs, and Waveforms}~\label{sec:rtds_and_waveforms}
...
If we assume that each reset removes the same amount of charge each time then we can rewrite the total charge out ($Q_{out}$) in terms of the integer number of resets at time t ($N(t)$):
\begin{equation}
Q_{out}(t) = Q_{o} N(t)
...
\begin{equation}
I_{o} = \frac{d}{dt}(Q_{o}N(t)) = Q_{o}\frac{dN}{dt} = \frac{Q_{o}f_{o}}{N_{clk}}
...
However, more interesting events deposit more charge as will be seen in Chapter~\ref{chap:sim}.
...
\subsection{Pixel Calibration}~\label{sec:qpix_calib}

The Q-Pix calibration scheme must use these timestamp to reproduce time, charge, and current. 
