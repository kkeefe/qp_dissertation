...
Since this time is much greater than the antipcated reset rates to be produced from backgrounds, discussed in a later section~\ref{sec:background}, we expect the looping of the 32-bit recorded value to not present a design concern.
...
We take a brief pause here to mention the inclusion of an optical system within the charge readout technology presented here.

Any reconstructed event requires some $T_{o}$ time to indicate the start of the event.
Typically this is done via scintillation photons, which arrive instantly at the collection planes compared to the slow drift speed of the electrons.

The pixel dimensions are 4 $\unit{mm}$ $\times$ 4 $\unit{mm} $ for a total active area of 16$\unit{mm^{2}}$.
Most of this active area is unused for the charge collection pad, which could be as small as drill-hole via.
Then, most of the remaining area could be plated with a photo-sensitive material, which can additionally be stored and readout.
...
Recent work has been accomplished to show viable plans to detect light within cold LAr temperatures~\citep{https://doi.org/10.48550/arxiv.2207.11127}.
