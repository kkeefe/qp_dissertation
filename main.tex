...
\begin{abstract}
The Standard Model (SM) of physics has proven successful in past decades, despite several measurements that hint at its incomplete description of nature.
The hunt for New Physics (NP) continues at higher energies ($\gg$~1~\unit{GeV}) with larger detectors ($\approx$~10~\unit{kT}).
One such future detector is The Deep Underground Neutrino Experiment (DUNE).
DUNE (as any beam line detector) is a combination of two detectors, a near-detector (ND) and a far-detector (FD) which will be used together to search for Charge-Parity violation (CPV) in the Lepton sector.
The DUNE-FD will be a combination of four large scale ($\approx$~10~\unit{kT}) Liquid Argon Time Projection Chambers (LArTPC).
Each 10-kT FD requires high precision in both timing ($\lg \mu s$) and spatial resolutions ($\approx$~1~\unit{mm}) for vertex reconstruction and particle identification (PID) of neutrino events.

This dissertation discusses the progress and characterization of a novel implementation of a new pixelated LArTPC readout technology which may be used in one FD.
...
This prototype is based on the first Q-Pix digital ASIC design, also presented in this work.
We discuss the back-end system requirements to for a Q-Pix based readout technology to provide neutrino oscillation measurements up to 10~\unit{GeV}, as well as present the first demonstration of local oscillator calibrations ($\sim$~0.1~\unit{ppm}).
Simulations have been performed based on radiogenic backgrounds and high-energy neutrino beamline events, providing first constraints of the digital back-end requirements in both the quiescent and active states.
